...
class BusMeter(Screens, Usage):
    ...
    def checkcrc(self, p1telegram):
        ...
        if givencrc != calccrc:
            pi.log_app.log_add(f"Error telegram checksum mismatch: {givencrc=}, {calccrc=}")
            return False
        return True
    ...
    def ts_obj(self, ts):
        ...
        if len(ts) != 13:
            pi.log_app.log_add(f"Error expecting 13 characters: {ts=}")
        if ts[12] not in ["S", "W"]:
            pi.log_app.log_add(f"Error expecting S or W at the end of {ts=}")
        try:
            dt = datetime.datetime.strptime(ts[:-1], '%y%m%d%H%M%S')
        except Exception as e:
            pi.log_app.log_add(f"Error parsing timestamp: {ts=} {e=}")
        ...
        match class_id:
            case 1 | 10:  # data
                ...
                if obis == "1-0:94.32.1":  # vgrid
                    if value not in ["230", "400"]:
                        pi.log_app.log_add(f"{obis}: Grid expecting 230 or 400: {value=}")
            ...
            case 7:  # profile generic
                ...
                if not all(x in BusMeter.obiscodes and BusMeter.obiscodes.get(x).class_id == 4 for x in ids):
                    pi.log_app.log_add(f"!!Expecting class_id == 4 -> {ids=} in {obis=}")
